...
function NavCat(){
	...
	while ($cr = mysqli_fetch_assoc($cres)) {
		$ret .= '<li><a href="category.php?url=' . $cr['caturl']. '">' .$cr['name'] .'</a></li>';
	}
	return $ret;
}
...
function PrintPagination($curpage, $startpage, $endpage, $previouspage, $nextpage, $url=null){
	...
	$ret .= '	  <ul class="pagination">';
		    if($curpage != $startpage){ 
				$ret .= '
				<li><a href="?'.$urlstring.'page='.$startpage.'" tabindex="-1" aria-label="Previous">
...
function SidebarComments(){
	global $connection;
	$comsql = "SELECT * FROM comments WHERE status='publish' ORDER by id DESC LIMIT 6";
  	$comres = mysqli_query($connection, $comsql);
  	if(mysqli_num_rows($comres)>=1){
	$ret ='<div class="panel panel-primary hidden-xs">
...
function SidebarArticles(){
	global $connection;
	$consql = "SELECT * FROM content WHERE status='published' ORDER BY id DESC LIMIT 5";
